...
export function TiltCard({
  ...
  const inner = (
    <Tilt
      ...
      <div
        className="flex flex-row transition-all duration-200 justify-between px-4 sm:px-6 py-4 sm:py-5">
        ...
        {price && badgeLabel ? (
          <div
            className="inline-flex h-fit items-center text-sm whitespace-nowrap shrink-0">
          ...
          <span
            className="h-fit rounded-full bg-secondary px-3 py-1 text-sm font-medium whitespace-nowrap shrink-0">
            {price}
          </span>
        ) : null}
      </div>
  ...
  if (href) {
    return (
      <a href={href} className="block cursor-pointer" {...(props)}>
        {inner}
      </a>
    );
  }
