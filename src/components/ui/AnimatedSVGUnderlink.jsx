...
export default function AnimatedSVGUnderlink({
  children,
  text,
  textColor,
  ...
  const scalePathCoordinates = useCallback((path, scaleX) => {
    return path.replace(
  ...
  const isHovered = externalIsHovered !== undefined ? externalIsHovered : localIsHovered;
  ...
  // When hover state becomes active, select next variant path
  useEffect(() => {
  ...
  const pathVariants = useMemo(() => ({
    ...
    visible: {
      pathLength: 1,
      pathOffset: 0,
      transition: prefersReducedMotion ? { duration: 0 } : animationTransition
    },
    erase: {
      pathLength: 1,
      pathOffset: 1,
      transition: prefersReducedMotion 
        ? { duration: 0 } 
        : { ...animationTransition, duration: animationTransition.duration || 0.45 }
    }
  }), [animationTransition, prefersReducedMotion]);
  ...
  return (
    <motion.span
      ...
      tabIndex={0}
    >
      <span ref={textRef} style={{ margin: 0, padding: 0 }}>
        {children || text}
      </span>
      <div
        style={{
